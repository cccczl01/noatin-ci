...
TEMPLATE="${SCRIPT_DIR}/DEBIAN/postinst.template"
...
示例:
  gen-postinst.sh --pkg-name chatgpt-client
  gen-postinst.sh --pkg-name chatgpt-client --output-dir /tmp/out
  gen-postinst.sh --pkg-name openclaw --has-desktop no --output-dir /tmp/out
EOF
    exit 1
}
...
if [[ -n "$OUTPUT_DIR" ]]; then
    ...
    OUTPUT_FILE="$OUTPUT_DIR/DEBIAN/postinst"
fi
